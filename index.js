...
var Main = function() {
	...
	var value = Main.vowels;
	if(__map_reserved["V"] != null) {
		_g.setReserved("V",value);
	} else {
		_g.h["V"] = value;
	}
	var value1 = Main.consonants;
	if(__map_reserved["C"] != null) {
		_g.setReserved("C",value1);
	} else {
		_g.h["C"] = value1;
	}
	var value2 = Main.all;
	if(__map_reserved["A"] != null) {
		_g.setReserved("A",value2);
	} else {
		_g.h["A"] = value2;
	}
	var value3 = Main.ending;
	if(__map_reserved["E"] != null) {
		_g.setReserved("E",value3);
	} else {
		_g.h["E"] = value3;
	}
	this.map = _g;
	this.resultList = [];
	this.parse("CVCVsoul.com",false);
};
Main.__name__ = true;
Main.main = function() {
	new Main();
...
Main.prototype = {
	parse: function(base,isFast) {
		...
		while(_g1 < _g) {
			var i = _g1++;
			var this1 = this.map;
			var key = base.charAt(i);
			var _this = this1;
			if(__map_reserved[key] != null ? _this.existsReserved(key) : _this.h.hasOwnProperty(key)) {
				var _g2 = 0;
				var this2 = this.map;
				var key1 = base.charAt(i);
				var _this1 = this2;
				var _g3 = __map_reserved[key1] != null ? _this1.getReserved(key1) : _this1.h[key1];
				while(_g2 < _g3.length) {
					var j = _g3[_g2];
					++_g2;
					var newBaseArr = base.split("");
					newBaseArr[i] = j;
					this.parse(newBaseArr.join(""),isFast);
					found = true;
				}
			}
		}
		if(!found) {
			this.lookup(base,isFast).handle(function(o) {
				if(o[1] == 0) {
					var domain = o[2];
					console.log(domain);
				}
	...
	,lookup: function(domain,isFast) {
		var _gthis = this;
		return tink_core__$Future_Future_$Impl_$.async(function(handler) {
			_gthis.lookupNative(domain,isFast,handler);
		});
	}
	,lookupNative: function(domain,isFast,handler) {
		var _gthis = this;
		Dns.resolve4(domain,function(err,data) {
			if(err != null || data == null || data.length == 0) {
				if(isFast) {
					handler(tink_core_Outcome.Success(domain));
				} else {
					_gthis.lookupWhoisNative(domain,handler);
				}
			}
		});
	}
	,lookupWhoisNative: function(domain,handler) {
		var _gthis = this;
		Whois.lookup(domain,function(err,data) {
			...
			if(err != null || data == null || data.indexOf("You have reached configured rate limit") != -1) {
				...
				handler(tink_core_Outcome.Failure("taken"));
			}
		});
	}
};
...
Main.consonants = ["b","c","d","f","g","h","j","k","l","m","n","p","q","r","s","t","v","w","x","y","z"];
Main.all = Main.vowels.concat(Main.consonants);
Main.ending = ["ful","ic","ous","ive","y","acy","al","ance","ence","dom","er","or","ism","ist","ity","ty","ment","ness","ship","sion","tion","ion"];
